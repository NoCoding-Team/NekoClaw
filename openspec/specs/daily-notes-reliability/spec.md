# daily-notes-reliability

每日笔记可靠性增强——启动补生成、LLM config fallback、时区统一和结构化日志。

---

## Overview

解决每日笔记 cron 在边缘场景下的可靠性问题：服务重启后自动补生成昨日笔记、用户无默认 LLM 配置时 fallback 到任意可用配置、时区统一为 UTC、增加结构化日志。
