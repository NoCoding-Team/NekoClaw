...
Digest 定时任务 SHALL 使用全局默认 LLM 配置执行 sub-LLM 调用。

#### Scenario: 使用全局默认 LLM
- **WHEN** Digest 任务执行
- **THEN** 系统 SHALL 从数据库读取全局默认 LLM 配置（`owner_id=None, is_default=True`），用于 sub-LLM 调用

#### Scenario: 无 LLM 配置时跳过
- **WHEN** 无全局默认 LLM 配置
- **THEN** 系统 SHALL 记录警告并跳过所有用户的 Digest 流程
