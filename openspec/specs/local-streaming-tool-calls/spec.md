...
`useLocalLLM` 的 `sendMessage` SHALL 实现 agentic while-loop：调用 LLM → 若返回 tool_calls → 执行工具 → 将结果追加到上下文 → 再次调用 LLM，直到 LLM 返回纯文本响应或达到轮次上限。`sendMessage` 在构建 LLM 上下文时 SHALL 保留完整的工具调用链路消息，包括 `role: 'tool'` 消息和带 `tool_calls` 的 assistant 消息。
