...
- [x] 13.2 创建定时任务数据库模型（服务端），同步到 PC 端本地存储
- [x] 13.3 实现定时任务创建 UI（一次性/周期性，时间选择器，任务描述）
- [ ] 13.4 实现任务触发逻辑：自动创建新会话并发送任务描述
- [x] 13.5 实现错过任务检测（PC 启动时检查）和补执行提示通知
- [x] 13.6 实现定时任务管理列表（暂停/删除/执行历史查看）
...
- [x] 14.5 配置 Electron 应用打包（Windows/macOS，包含 Playwright 依赖）
