...
_OPENAI_STREAM_PROVIDERS = {"openai", "openrouter"}
...
def _build_auth_headers(provider: str, api_key: str, original_headers: dict) -> dict:
    headers = {}
    skip = {"host", "content-length", "transfer-encoding", "authorization", "x-api-key", "accept-encoding"}
...
async def _check_quota(org_key_id: str, org_limit: int | None, sys_limit: int | None, db) -> tuple[bool, str]:
    ...
    if org_limit is not None and total_used >= org_limit:
        return False, f"org quota exhausted ({total_used}/{org_limit} tokens)"
    if sys_limit is not None and total_used >= sys_limit:
        return False, f"system quota exhausted ({total_used}/{sys_limit} tokens)"
    return True, ""
...
async def llm_proxy(provider: str, path: str, request: Request):
    ...
    async with get_session() as db:
        ...
        if config is None:
            return JSONResponse(status_code=404, content={
                "error": f"No LLM key configured for {provider}"
            })
        ...
        if is_org_key:
            ...
            if org_key is None:
                return JSONResponse(status_code=404, content={
                    "error": f"No org key configured for {provider}"
                })
            ...
            if user_key is None:
                return JSONResponse(status_code=404, content={
                    "error": f"No personal key found for {provider}"
                })
    ...
    ctx = _RequestContext(
        instance=instance, provider=provider, key_source=key_source,
        org_key_id=org_key_id, request_path=f"/{path}", is_stream=is_stream,
        raw_body=raw_body,
    )
...
async def _handle_non_stream(
    ...
    if resp_body:
        try:
            ...
            return JSONResponse(status_code=resp.status_code, content=parsed, headers=resp_headers)
        except (json.JSONDecodeError, UnicodeDecodeError):
            from fastapi.responses import Response
            return Response(
                status_code=resp.status_code, content=resp_body,
                headers=resp_headers,
                media_type=resp.headers.get("content-type", "application/json"),
            )
    return JSONResponse(status_code=resp.status_code, content=None, headers=resp_headers)
...
async def _record_usage(
    ctx: _RequestContext, *, usage: dict,
    status_code: int | None = None, latency_ms: int | None = None,
