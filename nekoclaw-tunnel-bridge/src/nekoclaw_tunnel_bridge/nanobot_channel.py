...
logger = logging.getLogger("nekoclaw_tunnel_bridge.nanobot")
...
class NekoClawyChannel(BaseChannel):  # type: ignore[misc]
    """NanoBot channel that connects to NekoClaw tunnel for workspace group chat."""

    name = "nekoclaw"
    display_name = "NekoClaw"
    ...
    async def start(self) -> None:
        if hasattr(self, "_client") and self._client:
            logger.warning("NekoClaw channel: stopping previous tunnel client before restart")
        ...
        self._running = True
        logger.info("NekoClaw channel starting tunnel client...")
    ...
    async def _handle_chat_request(
        ...
        session_key = f"nekoclaw:{workspace_id}" if workspace_id else None
