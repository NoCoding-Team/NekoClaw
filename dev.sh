...
BACKEND_DIR="$SCRIPT_DIR/nekoclaw-backend"
LLM_PROXY_DIR="$SCRIPT_DIR/nekoclaw-llm-proxy"
PORTAL_DIR="$SCRIPT_DIR/nekoclaw-portal"
ADMIN_DIR="$EE_DIR/nekoclaw-frontend"
...
FRESH=false
DOCKER_PG=false
DOCKER_PG_CONTAINER="nekoclaw-pg"
DOCKER_PG_VOLUME="nekoclaw_pg_dev"
...
选项:
  --fresh      强制重新安装依赖
  --docker-pg  使用 Docker 启动本地 PostgreSQL
  --help       显示本帮助
...
if [ "$DOCKER_PG" = true ]; then
  ...
  if docker ps --format '{{.Names}}' | grep -q "^${DOCKER_PG_CONTAINER}$"; then
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "^${DOCKER_PG_CONTAINER}$"; then
      ...
      docker run -d --name "$DOCKER_PG_CONTAINER" \
        -p 5432:5432 \
        -e POSTGRES_USER=nekoclaw \
        -e POSTGRES_PASSWORD=nekoclaw \
        -e POSTGRES_DB=nekoclaw \
    ...
    for i in $(seq 1 30); do
      if docker exec "$DOCKER_PG_CONTAINER" pg_isready -U nekoclaw &>/dev/null; then
        break
      fi
      if [ "$i" -eq 30 ]; then
        err "PostgreSQL 启动超时"
  ...
  export DATABASE_URL="postgresql+asyncpg://nekoclaw:nekoclaw@localhost:5432/nekoclaw"
...
if [ ! -f "$BACKEND_DIR/.env" ]; then
  ...
  echo "  请先复制并配置: cp nekoclaw-backend/.env.example nekoclaw-backend/.env"
  exit 1
fi
...
echo "${BOLD}========================================${RESET}"
echo "${BOLD} NekoClaw 本地开发环境 (${MODE_UPPER})${RESET}"
echo "${BOLD}========================================${RESET}"
