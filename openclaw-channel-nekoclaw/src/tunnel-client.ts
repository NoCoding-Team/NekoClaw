...
export function startTunnelClient(cfg: OpenClawConfig): TunnelClient {
  ...
  const section = (cfg as Record<string, unknown>).channels?.["nekoclaw"] as
...
export class TunnelClient {
  ...
  private handleMessage(msg: TunnelMessage): void {
    switch (msg.type) {
      ...
      case "auth.error":
        console.error("[tunnel] Auth failed:", msg.payload?.reason ?? "unknown");
        this.closed = true;
        this.ws?.close();
        break;
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        this.send({
          ...
          replyTo: msg.id,
          traceId: msg.traceId,
          payload: { error: `Gateway error ${resp.status}: ${errorText}` },
          ts: Date.now(),
        });
        return;
      }
  ...
  private handleLearningTask(msg: TunnelMessage): void {
    if (!this.learningHandler) {
      console.warn("[tunnel] Learning task received but no handler registered");
      return;
    }
    try {
      const result = this.learningHandler(msg.payload);
      console.log("[tunnel] Learning task handled:", result);
    } catch (err) {
  ...
  private startPingCheck(): void {
    ...
    this.pingCheckTimer = setInterval(() => {
      if (Date.now() - this.lastPong > PONG_TIMEOUT_MS) {
        console.warn("[tunnel] Pong timeout, reconnecting...");
        this.cleanup();
        if (!this.closed) {
          this.connect();
        }
  ...
  private scheduleReconnect(): void {
    ...
    console.log(
      `[tunnel] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempt})`,
    );
