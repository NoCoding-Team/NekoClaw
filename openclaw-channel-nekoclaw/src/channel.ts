...
import type {
  NekoClawyAccountConfig,
  ResolvedNekoClawyAccount,
  CollaborationPayload,
} from "./types.js";
import { getNekoClawyRuntime } from "./runtime.js";
import { getTunnelClient, isProtocolDowngraded } from "./tunnel-client.js";

const CHANNEL_KEY = "nekoclaw";
...
function resolveAccount(
  ...
  const accounts = (section?.accounts ?? {}) as Record<string, NekoClawyAccountConfig>;
...
async function listNekoClawyPeers(params: {
  cfg: OpenClawConfig;
  accountId?: string | null;
  query?: string | null;
  limit?: number | null;
  ...
  const effectiveUrl = isProtocolDowngraded()
    ...
    : account.apiUrl;
  try {
    const url = `${effectiveUrl}/api/v1/workspaces/${account.workspaceId}/topology/reachable?instance_id=${account.instanceId}`;
...
export const nekoclawPlugin: ChannelPlugin<ResolvedNekoClawyAccount> = {
  id: CHANNEL_KEY,
  meta: {
    id: CHANNEL_KEY,
    label: "NekoClaw",
    selectionLabel: "NekoClaw (AI Cat Companion)",
    docsPath: "/channels/nekoclaw",
    blurb: "NekoClaw AI cat companion workspace channel.",
    aliases: ["neko", "cat"],
  },
  capabilities: {
    chatTypes: ["direct"],
  ...
  outbound: {
    ...
    sendText: async ({ cfg, to, text, accountId }) => {
      ...
      getNekoClawyRuntime().channel.activity.record({
        channel: CHANNEL_KEY,
        accountId: account.accountId,
        direction: "outbound",
      });

      const messageId = `neko-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  directory: {
    listPeers: (params) => listNekoClawyPeers(params),
    listPeersLive: (params) => listNekoClawyPeers(params),
  },
  messaging: {
  ...
  agentPrompt: {
    messageToolHints: () => [
      `Use "send -t nekoclaw -to \\"agent:{name}\\" -m \\"message\\"" to collaborate with other AI cat companions.`,
      `To message a reachable human partner: send -t nekoclaw -to "human:{name}" -m "message". The {name} is the display_name shown in get_my_neighbors.`,
      ...
      `If get_my_neighbors returns a blackboard (node_type=blackboard), you can interact with it via the nekoclaw_workspace tool. The blackboard is for shared tasks and objectives — do NOT use send to message it.`,
    ],
  },
  status: {
