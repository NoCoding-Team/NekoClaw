...
class Gene(BaseModel):
    __tablename__ = "genes"
    __table_args__ = (
        Index("uq_genes_slug_org_active", "slug", "org_id", unique=True, postgresql_where="deleted_at IS NULL"),
    )
    ...
    source: Mapped[str] = mapped_column(String(16), default=GeneSource.official, nullable=False)
    ...
    synergies: Mapped[str | None] = mapped_column(Text, nullable=True)

    parent_gene_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("genes.id"), nullable=True)
    created_by_instance_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("instances.id"), nullable=True)
    ...
    visibility: Mapped[str] = mapped_column(
        String(16), default=ContentVisibility.public, nullable=False, server_default="public",
    )
...
class Genome(BaseModel):
    ...
    __table_args__ = (
        Index("uq_genomes_slug_org_active", "slug", "org_id", unique=True, postgresql_where="deleted_at IS NULL"),
    )
    ...
    short_description: Mapped[str | None] = mapped_column(String(256), nullable=True)
    icon: Mapped[str | None] = mapped_column(String(32), nullable=True)
    gene_slugs: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class InstanceGene(BaseModel):
    ...
    __table_args__ = (
        Index("uq_instance_gene_active", "instance_id", "gene_id", unique=True, postgresql_where="deleted_at IS NULL"),
    )

    instance_id: Mapped[str] = mapped_column(String(36), ForeignKey("instances.id"), nullable=False, index=True)
    gene_id: Mapped[str] = mapped_column(String(36), ForeignKey("genes.id"), nullable=False, index=True)
    genome_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("genomes.id"), nullable=True)
    status: Mapped[str] = mapped_column(String(20), default=InstanceGeneStatus.installing, nullable=False)
    ...
    variant_published: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class GeneRating(BaseModel):
    ...
    __table_args__ = (
        Index("uq_gene_rating_user", "gene_id", "user_id", unique=True, postgresql_where="deleted_at IS NULL"),
    )
...
class GenomeRating(BaseModel):
    ...
    __table_args__ = (
        Index("uq_genome_rating_user", "genome_id", "user_id", unique=True, postgresql_where="deleted_at IS NULL"),
    )

    genome_id: Mapped[str] = mapped_column(String(36), ForeignKey("genomes.id"), nullable=False, index=True)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"), nullable=False)
    rating: Mapped[int] = mapped_column(Integer, nullable=False)
    comment: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class EvolutionEvent(BaseModel):
    ...
    instance_id: Mapped[str] = mapped_column(String(36), ForeignKey("instances.id"), nullable=False, index=True)
    gene_id: Mapped[str | None] = mapped_column(String(36), nullable=True, index=True)
