...
NekoClaw 是猫咪 AI 经营伙伴管理平台，通过 Web 界面管理 K8s 集群上的 AI 猫咪实例。
...
| LLM Proxy | Python + FastAPI | CE + EE |
...
### 前端（ee/nekoclaw-frontend / nekoclaw-portal）

```bash
cd ee/nekoclaw-frontend  # 管理后台（EE-only）
npm install
npm run dev              # http://localhost:4518
npm run build

cd nekoclaw-portal       # 用户门户
npm install
npm run dev              # http://localhost:4517
npm run build
npm run test             # vitest
npm run test -- --run src/components/xxx.spec.ts
```
...
| 逻辑类型 | 可能位置 |
|---------|----------|
| slug 生成、表单校验 | `ee/nekoclaw-frontend` 和 `nekoclaw-portal` 的对应页面 |
...
- `ee/` — EE 私有仓库（`.gitignore` 排除，开发者通过 `scripts/setup-ee.sh` 拉取）
  ...
  - `ee/nekoclaw-frontend/` — Admin 管理后台前端（EE-only，完整 Vue 项目）
...
- **Admin**（`ee/nekoclaw-frontend/`）：完整独立的 Vue 项目，EE-only，CE 版不包含此目录。EE 路由直接定义在 `src/router/index.ts` 中。
- **Portal**（`nekoclaw-portal/`）：CE + EE 共用。CE 前端定义 `src/router/ee-stub.ts`（空数组），Vite 在检测到 `ee/` 时通过 alias 替换为 `ee/frontend/portal/routes.ts` 提供的 EE 路由。
